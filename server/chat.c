...
char *CHATcreateJSON ( int *a_statusCode, char *a_sender, char *a_room, char *a_message, char *aout_reply ) {
    ...
    snprintf(statusJSON, sizeof(statusJSON), " \"status\": %d,", *a_statusCode);
    if (a_sender) {
        snprintf(senderJSON, sizeof(senderJSON), " \"sender\": \"%s\",", a_sender);
    }
    if (a_room) {
        snprintf(roomJSON, sizeof(roomJSON), " \"room\": \"%s\",", a_room);
    }
    if (a_message[0] == '[') {
    ...
    snprintf(aout_reply, sizeof(aout_reply), "{ %s %s %s %s }", statusJSON, senderJSON, roomJSON, messageJSON);
...
void CHATremoveUser(char *a_name, int *a_soc, int *a_pos) {
    ...
    snprintf(controlMessage, sizeof(controlMessage), "User %s has logged out.", room.param);
...
int CHATassignToRoom(int a_id, int *a_fd) {
    ...
    if ((*SHM).tabRoom[a_id].users == MAX_USERS) {
        ...
        (*SHM).tabRoom[a_id].id = a_id + 1;  // TO DO
